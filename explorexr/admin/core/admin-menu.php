...
function expoxr_fix_admin_menu_highlighting($parent_file) {
    ...
    if (isset($_GET['page']) && $_GET['page'] === 'expoxr-edit-model') {
...
function expoxr_ajax_dismiss_premium_banner() {
    // Verify nonce
    if (!wp_verify_nonce($_POST['nonce'], 'expoxr_dashboard_nonce')) {
...
function expoxr_ajax_get_premium_info() {
    // Verify nonce
    if (!wp_verify_nonce($_POST['nonce'], 'expoxr_admin_nonce')) {
...
function expoxr_ajax_clear_all_debug_logs() {
    // Verify nonce
    if (!wp_verify_nonce($_POST['nonce'], 'expoxr_clear_logs')) {
        wp_send_json_error('Security check failed');
    }
...
function expoxr_ajax_run_diagnostics() {
    // Verify nonce
    if (!wp_verify_nonce($_POST['nonce'], 'expoxr_diagnostics')) {
        wp_send_json_error('Security check failed');
    }
...
function expoxr_ajax_export_debug_info() {
    // Verify nonce
    if (!wp_verify_nonce($_POST['nonce'], 'expoxr_export_debug')) {
        wp_send_json_error('Security check failed');
    }
