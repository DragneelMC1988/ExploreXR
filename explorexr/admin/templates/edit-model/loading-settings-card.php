...
// Per-model lazy load: always available in free version
// phpcs:disable WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedVariableFound -- Template variables
$lazy_load = get_post_meta($model_id, '_explorexr_lazy_load', true) ?: 'global';
...
<div class="explorexr-card">
    ...
    <div class="explorexr-card-content">
        <div class="explorexr-form-grid explorexr-form-grid-3">
            <!-- Per-model lazy load (free) -->
            <div class="explorexr-form-group">
                <label for="explorexr_lazy_load"><?php esc_html_e('Lazy Load Model', 'explorexr'); ?></label>
                <select name="explorexr_lazy_load" id="explorexr_lazy_load">
                    <option value="global" <?php selected($lazy_load, 'global'); ?>><?php esc_html_e('Use Global Setting', 'explorexr'); ?></option>
                    <option value="lazy" <?php selected($lazy_load, 'lazy'); ?>><?php esc_html_e('Always Lazy', 'explorexr'); ?></option>
                    <option value="eager" <?php selected($lazy_load, 'eager'); ?>><?php esc_html_e('Always Eager', 'explorexr'); ?></option>
                </select>
                <p class="description"><?php esc_html_e('Override the global lazy load setting for this model. "Use Global Setting" defers to ExploreXR \u2192 Loading Options.', 'explorexr'); ?></p>
            </div>
        </div>
    </div>
</div>

<?php if (function_exists('explorexr_is_premium_available') && explorexr_is_premium_available()):
// Get loading settings -- premium-only fields
...
<div class="explorexr-card">
    <div class="explorexr-card-header">
        <h2><span class="dashicons dashicons-performance"></span> Advanced Loading Settings</h2>
    </div>
    <div class="explorexr-card-content">
        <div class="explorexr-form-grid explorexr-form-grid-3">
            <div class="explorexr-form-group">
            ...
            <div class="explorexr-form-group">
                ...
                <p class="description">Where to load model viewer scripts</p>
            </div>
        </div>
    </div>
</div>
<?php endif; // End premium loading customization check ?>
