...
function expoxr_browse_models_page() {
    ...
    <div class="wrap expoxr-admin-container">
        ...
        if (isset($_GET['created']) && $_GET['created'] === 'true') {
            ...
            if ($creation_result) {
                ...
                ?>
                <div class="expoxr-alert <?php echo esc_attr($alert_class); ?>">
            ...
            <div class="expoxr-card-content">
                <?php if (empty($models)) : ?>
                    <div class="expoxr-alert info">
                        ...
                        <div>
                            <p>You don't have any 3D models yet. <a href="<?php echo esc_url(admin_url('admin.php?page=expoxr-create-model')); ?>">Create your first 3D model</a>.</p>
                        </div>
                    </div>
                <?php else : ?>
                    <div class="expoxr-filter-bar">
                        <div class="expoxr-search-box">
                            <input type="text" id="model-search" placeholder="Search models...">
                            <button type="button" class="button"><span class="dashicons dashicons-search"></span></button>
                        </div>
                        <div class="expoxr-sort-options">
                    ...
                    <div class="expoxr-models-grid">                        <?php foreach ($models as $model) : 
                            $model_file = get_post_meta($model->ID, '_expoxr_model_file', true);
                            $model_alt_file = get_post_meta($model->ID, '_expoxr_model_alt_file', true);
                            $poster = get_post_meta($model->ID, '_expoxr_model_poster', true);
                            $auto_rotate = get_post_meta($model->ID, '_expoxr_auto_rotate', true) === 'on';
                            $camera_controls = get_post_meta($model->ID, '_expoxr_camera_controls', true) === 'on';
                            $ar_enabled = get_post_meta($model->ID, '_expoxr_model_ar', true);
                            $shortcode = '[expoxr_model id="' . $model->ID . '"]';
                        ?>
                            <div class="expoxr-model-card" data-title="<?php echo esc_attr($model->post_title); ?>" data-date="<?php echo esc_attr($model->post_date); ?>">
                                <div class="expoxr-model-preview">
                                    ...
                                    <?php else : ?>
                                        <div class="expoxr-no-preview">
                                            <span class="dashicons dashicons-format-image"></span>
                                            <p>No model preview</p>
                                        </div>
                                    <?php endif; ?>
                                </div>                                <div class="expoxr-model-info">
    ...
    <div id="expoxr-copied-notification" style="display: none; position: fixed; bottom: 20px; right: 20px; background-color: #2271b1; color: white; padding: 10px 20px; border-radius: 4px; box-shadow: 0 2px 8px rgba(0,0,0,0.2); z-index: 9999;">
