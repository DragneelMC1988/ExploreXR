...
// Run verification if accessed directly via CLI or as a standalone script
// Never output during normal WordPress operation
if (defined('WP_CLI') && WP_CLI || (isset($_SERVER['PHP_SELF']) && basename(sanitize_text_field(wp_unslash($_SERVER['PHP_SELF']))) === basename(__FILE__) && (!function_exists('is_admin') || !is_admin()))) {
    ?>
    <!DOCTYPE html>
    <html>
    <head>
